 ...
 * Fixed for Big-Endian (s390x) support.
 * @module
 */
import { add3H, add3L, rotr32H, rotr32L, rotrBH, rotrBL, rotrSH, rotrSL } from './_u64.ts';
import { blake2b } from './blake2.ts';
import { anumber, clean, kdfInputToBytes, nextTick, u32, u8, swap32IfBE, isLE, type KDFInput } from './utils.ts';
...
function P(
  ...
  G(v00, v04, v08, v12); G(v01, v05, v09, v13); G(v02, v06, v10, v14); G(v03, v07, v11, v15);
  G(v00, v05, v10, v15); G(v01, v06, v11, v12); G(v02, v07, v08, v13); G(v03, v04, v09, v14);
}
...
// Fixed for Big-Endian: Ensure DataView/Casting is LE
function Hp(A: Uint32Array, dkLen: number) {
  swap32IfBE(A); // Normalize to LE for hashing
  const A8 = u8(A);
  const T = new Uint32Array(1);
  T[0] = dkLen;
  swap32IfBE(T); // Ensure dkLen is LE in memory
  const T8 = u8(T);

  let res: Uint8Array;
  if (dkLen <= 64) {
    res = blake2b.create({ dkLen }).update(T8).update(A8).digest();
  } else {
  ...
  swap32IfBE(A); // Revert A to host endianness
  clean(T);
  return u32(res);
}

function indexAlpha(r: number, s: number, laneLen: number, segmentLen: number, index: number, randL: number, sameLane: boolean = false) {
  let area: number;
  if (r === 0) {
    if (s === 0) area = index - 1;
...
export type ArgonOpts = {
  t: number; m: number; p: number; version?: number; key?: KDFInput;
  personalization?: KDFInput; dkLen?: number; asyncTick?: number;
  maxmem?: number; onProgress?: (progress: number) => void;
};

const maxUint32 = Math.pow(2, 32);
function isU32(num: number) { return Number.isSafeInteger(num) && num >= 0 && num < maxUint32; }

function argon2Opts(opts: ArgonOpts) {
  const merged: any = { version: 0x13, dkLen: 32, maxmem: maxUint32 - 1, asyncTick: 10 };
...
function argon2Init(password: KDFInput, salt: KDFInput, type: Types, opts: ArgonOpts) {
  ...
  const BUF8 = u8(BUF);
  // Fixed for BE: BUF[0] must be LE in memory before update()
  ...
  h.digestInto(u8(H0));
  swap32IfBE(H0); // Convert digest words back to host endianness

  const mP = 4 * p * Math.floor(m / (ARGON2_SYNC_POINTS * p));
  const laneLen = Math.floor(mP / p);
  const segmentLen = Math.floor(laneLen / ARGON2_SYNC_POINTS);
  const memUsed = mP * 256;
  if (memUsed > maxmem) throw new Error('"maxmem" exceeded');
  ...
  if (onProgress) {
    ...
    perBlock = () => {
      blockCnt++;
      if (!(blockCnt % callbackPer) || blockCnt === totalBlock) onProgress(blockCnt / totalBlock);
    };
  }
  clean(BUF, H0);
  return { type, mP, p, t, version, B, laneLen, lanes: p, segmentLen, dkLen, perBlock, asyncTick };
}
...
function argon2Output(B: Uint32Array, p: number, laneLen: number, dkLen: number) {
  ...
  // Hp handles endianness internally now
  const res = u8(Hp(B_final, dkLen));
  clean(B_final);
  return res;
}

function processBlock(B: Uint32Array, address: Uint32Array, l: number, r: number, s: number, index: number, laneLen: number, segmentLen: number, lanes: number, offset: number, prev: number, dataIndependent: boolean, needXor: boolean) {
...
function argon2(type: Types, password: KDFInput, salt: KDFInput, opts: ArgonOpts) {
  ...
  address[256 + 6] = mP; address[256 + 8] = t; address[256 + 10] = type;
  for (let r = 0; r < t; r++) {
    ...
    for (let s = 0; s < ARGON2_SYNC_POINTS; s++) {
      ...
      for (let l = 0; l < p; l++) {
        address[256 + 2] = l; address[256 + 12] = 0;
        let startPos = (r === 0 && s === 0) ? 2 : 0;
        if (r === 0 && s === 0 && dataIndependent) {
          address[256 + 12]++;
          block(address, 256, 2 * 256, 0, false);
          block(address, 0, 2 * 256, 0, false);
        }
...
export const argon2d = (p: KDFInput, s: KDFInput, o: ArgonOpts) => argon2(AT.Argond2d, p, s, o);
export const argon2i = (p: KDFInput, s: KDFInput, o: ArgonOpts) => argon2(AT.Argon2i, p, s, o);
export const argon2id = (p: KDFInput, s: KDFInput, o: ArgonOpts) => argon2(AT.Argon2id, p, s, o);
...
export const argon2dAsync = (p: KDFInput, s: KDFInput, o: ArgonOpts) => argon2Async(AT.Argond2d, p, s, o);
export const argon2iAsync = (p: KDFInput, s: KDFInput, o: ArgonOpts) => argon2Async(AT.Argon2i, p, s, o);
export const argon2idAsync = (p: KDFInput, s: KDFInput, o: ArgonOpts) => argon2Async(AT.Argon2id, p, s, o);
